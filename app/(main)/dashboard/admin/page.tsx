...
import { Users, Package, ShoppingBag, Search, ChevronLeft, ChevronRight, Loader2, ShieldCheck, ShieldAlert } from "lucide-react";
...
export default function AdminDashboardPage() {
  const [stats, setStats] = useState({ totalUsers: 0, activeGear: 0, totalRentals: 0, totalRevenue: 0 });
  ...
  const fetchAdminData = useCallback(async () => {
    try {
      ...
      const [statsRes, usersRes] = await Promise.all([
        ...
        api.get(`/api/admin/users?search=${encodeURIComponent(search)}&page=${page}&limit=8`, config),
      ]);
  ...
  const handleToggleStatus = async (userId: string, currentBlocked: boolean) => {
    setUpdatingId(userId);
    try {
      const token = Cookies.get("accessToken");
      const config = token ? { headers: { Authorization: `Bearer ${token}` } } : {};

      await api.patch(`/api/admin/users/${userId}`, { isBlocked: !currentBlocked }, config);
      toast.success(`User ${!currentBlocked ? "suspended" : "activated"} successfully`);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 py-6">
      <div>
        ...
        <p className="text-sm text-zinc-500">Monitor platform operations and enforce moderation policies.</p>
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-zinc-200 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
          <div>
            ...
            <p className="text-xs text-zinc-500">Search, inspect roles, and suspend malicious accounts.</p>
          </div>
                  ...
                  <tr key={u.id} className="hover:bg-zinc-50/50">
                    ...
                    <td className="p-4 text-right">
                      <button
                        ...
                        onClick={() => handleToggleStatus(u.id, u.isBlocked)}
